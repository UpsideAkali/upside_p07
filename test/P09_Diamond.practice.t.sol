...
contract DiamondPracticeTest {
    ...
    function _registerInitialFacets(
        Diamond diamond,
        bytes4[] memory ownerSels,
        OwnerFacet ownerFacet,
        bytes4[] memory counterSels,
        CounterFacetV1 counterV1
    ) internal {
        IDiamondCut(address(diamond)).diamondCut(ownerSels, address(ownerFacet));
        IDiamondCut(address(diamond)).diamondCut(counterSels, address(counterV1));
    }
    ...
    function _assertPostUpgradeFlow(Diamond diamond) internal {
        ...
        require(
            ...
            "increment should work after upgrade"
        );
